...
use cra::agent::plasticity::{PlasticState, PlasticityError};
...
fn seeded_state(
    ...
    let state = PlasticState::new(topology, w0, "all_recurrent_edges", "persistent", 32.0).unwrap();
...
fn delta_uses_old_baseline_and_updates_once_after() {
    ...
    let out = state
        .apply_feedback_once(0, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.1, &w0)
        .unwrap();
    ...
    let mut snap = state.snapshot();
    snap.e[2][0] = 1.0;
    let mut state2 = PlasticState::restore(snap, &topology, &w0).unwrap();
    let out2 = state2
        .apply_feedback_once(1, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.1, &w0)
        .unwrap();
    close(
        out2.delta,
        1.0 - 0.64,
...
fn duplicate_and_older_events_leave_all_state_unchanged() {
    ...
    state
        .apply_feedback_once(3, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.02, &w0)
        .unwrap();
    ...
    assert!(matches!(
        state.apply_feedback_once(3, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.02, &w0),
        Err(PlasticityError::DuplicateFeedback(3))
    ));
    // An older id is also rejected (monotonic dedup, as in B3).
    assert!(matches!(
        state.apply_feedback_once(1, 0.0, &ones(4), 0.01, 0.01, 0.5, 0.02, &w0),
    ...
    let out = state
        .apply_feedback_once(4, 0.0, &ones(4), 0.01, 0.01, 0.5, 0.02, &w0)
        .unwrap();
...
fn zero_eta_zero_gate_and_zero_delta_leave_p_unchanged() {
    ...
    let out = state
        .apply_feedback_once(0, 1.0, &ones(4), 0.0, 0.01, 0.5, 0.1, &w0)
        .unwrap();
    ...
    let mut state = seeded_state(&topology, &w0, (2, 0), 1.5, 0.03, 0.5);
    let out = state
        .apply_feedback_once(0, 1.0, &[0.0; 4], 0.01, 0.01, 0.5, 0.1, &w0)
        .unwrap();
    assert!(out.actual_updates.iter().flatten().all(|&v| v == 0.0));
    assert_eq!(state.p()[2][0], 0.03);
...
fn max_update_clipping_separates_raw_from_limited() {
    ...
    let out = state
        .apply_feedback_once(0, 1.0, &ones(4), 0.1, 0.01, 0.5, 0.0, &w0)
        .unwrap();
    ...
    let mut state = seeded_state(&topology, &w0, (2, 0), 2.0, 0.0, 0.5);
    let out = state
        .apply_feedback_once(0, 0.0, &ones(4), 0.1, 0.01, 0.5, 0.0, &w0)
        .unwrap();
...
fn plastic_bound_clipping_separates_limited_from_actual() {
    ...
    let out = state
        .apply_feedback_once(0, 1.0, &ones(4), 0.1, 1.0, 0.5, 0.0, &w0)
        .unwrap();
    ...
    let mut state = seeded_state(&topology, &w0, (2, 0), 1.0, -0.49, 0.5);
    let out = state
        .apply_feedback_once(0, 0.0, &ones(4), 0.1, 1.0, 0.5, 0.0, &w0)
        .unwrap();
...
fn w0_never_changes_and_effective_stays_w0_plus_p() {
    ...
    let out = state
        .apply_feedback_once(7, 1.0, &ones(4), 0.02, 0.01, 0.5, 0.05, &w0)
        .unwrap();
...
fn nonplastic_edges_never_acquire_reports_or_offsets() {
    ...
    snap.e[2][0] = 1.2;
    snap.e[3][2] = -0.9;
    let mut state = PlasticState::restore(snap, &topology, &w0).unwrap();
    let out = state
        .apply_feedback_once(0, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.05, &w0)
        .unwrap();
...
fn gate_scaling_is_per_receiver_and_doubling_doubles_unclipped_updates() {
    ...
    snap.e[2][0] = 1.0;
    snap.e[2][3] = 2.0;
    let mut half = PlasticState::restore(snap.clone(), &topology, &w0).unwrap();
    let mut full = PlasticState::restore(snap, &topology, &w0).unwrap();
    let mut gates_half = ones(4);
    gates_half[2] = 0.5;
    let half_out = half
        .apply_feedback_once(0, 1.0, &gates_half, 0.01, 1.0, 0.5, 0.0, &w0)
        .unwrap();
    let full_out = full
        .apply_feedback_once(0, 1.0, &ones(4), 0.01, 1.0, 0.5, 0.0, &w0)
        .unwrap();
    close(
    ...
    let mut other = PlasticState::restore(full.snapshot(), &topology, &w0).unwrap();
    let mut gates_other = vec![0.0; 4];
    gates_other[1] = 1.0;
    let other_out = other
        .apply_feedback_once(1, 1.0, &gates_other, 0.01, 1.0, 0.5, 0.0, &w0)
        .unwrap();
...
fn invalid_feedback_inputs_are_rejected_without_state_change() {
    ...
    for &reward in &[0.5, 2.0, -1.0, f64::NAN, f64::INFINITY] {
        assert!(
            state
                .apply_feedback_once(10, reward, &ones(4), 0.01, 0.01, 0.5, 0.05, &w0)
                .is_err(),
            "reward {reward}"
        );
        bad_calls += 1;
    ...
    assert!(
        state
            .apply_feedback_once(10, 1.0, &[1.0; 3], 0.01, 0.01, 0.5, 0.05, &w0)
            .is_err()
    );
    bad_calls += 1;
    for gates in [
        ...
        assert!(
            state
                .apply_feedback_once(10, 1.0, &gates, 0.01, 0.01, 0.5, 0.05, &w0)
                .is_err(),
            "gates {gates:?}"
        );
        bad_calls += 1;
    ...
    for (eta, max_update, bound, beta) in [
        (-0.01, 0.01, 0.5, 0.05),
        (f64::NAN, 0.01, 0.5, 0.05),
        (0.01, 0.0, 0.5, 0.05),
        (0.01, -0.01, 0.5, 0.05),
        (0.01, 0.01, 0.0, 0.05),
        (0.01, 0.01, 0.5, -0.1),
        ...
        assert!(
            state
                .apply_feedback_once(10, 1.0, &ones(4), eta, max_update, bound, beta, &w0)
                .is_err(),
            "hyperparams {eta}/{max_update}/{bound}/{beta}"
        );
        bad_calls += 1;
    }
    ...
    assert!(
        state
            .apply_feedback_once(10, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.05, &ragged)
            .is_err()
    );
    bad_calls += 1;
    ...
    assert!(
        state
            .apply_feedback_once(10, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.05, &bad_w0)
            .is_err()
    );
    bad_calls += 1;
    assert!(bad_calls > 10);
    ...
    state
        .apply_feedback_once(10, 1.0, &ones(4), 0.01, 0.01, 0.5, 0.05, &w0)
        .unwrap();
...
fn snapshot_preserves_baseline_dedup_and_nonzero_p() {
    ...
    state
        .apply_feedback_once(5, 1.0, &ones(4), 0.02, 0.01, 0.5, 0.05, &w0)
        .unwrap();
    ...
    assert!(matches!(
        restored.apply_feedback_once(5, 1.0, &ones(4), 0.02, 0.01, 0.5, 0.05, &w0),
